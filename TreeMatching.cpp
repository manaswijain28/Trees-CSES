...
void solve(){
    ...
    function<int(int,int,int)> treeMatching = [&](int node,int parent,int include)->int{
        if(dp[node][include] != -1){
            return dp[node][include];
        }
        ...
        if(include){
            ...
            for(auto it : adj[node]){
                if(it != parent){
                    fin = max(fin,ans-dp[it][1]+1+treeMatching(it,node,0));
                }
            }
            ...
            for(auto it : adj[node]){
                if(it != parent){
                    ...
                    int x = treeMatching(it,node,1);
                    ans += max(x,treeMatching(it,node,0));
                }
            }
    ...
    cout<<max(treeMatching(1,-1,0),treeMatching(1,-1,1));
